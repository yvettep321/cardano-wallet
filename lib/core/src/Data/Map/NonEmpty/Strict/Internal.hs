...
-- This instance prevents the definition of a 'Generic' instance, as this could
-- be used to subvert the invariant, leading to undefined behaviour.
--
-- An example of where this might happen is the generation of arbitrary data in
-- a QuickCheck test, where someone might be tempted to use 'genericShrink' to
-- shrink a map.
--
instance (TypeError ('Text "Generic not supported for NonEmptyMap.")) =>
    Generic (NonEmptyMap k v) where
        type Rep (NonEmptyMap k v) = (,) Void
        from = error "Generic not supported for NonEmptyMap."
        to   = error "Generic not supported for NonEmptyMap."
